...
import { MeilisearchIndexerController, ReindexMessageResponse } from './indexer.controller';

@Injectable()
export class MeilisearchIndexService implements OnApplicationBootstrap {
    private updateIndexQueue: JobQueue<UpdateIndexQueueJobData>;

    constructor(
        private jobService: JobQueueService,
        private indexerController: MeilisearchIndexerController,
    ) {}
    ...
    async onApplicationBootstrap() {
        this.updateIndexQueue = await this.jobService.createQueue({
            ...
            process: job => {
                ...
                switch (data.type) {
                    case 'reindex':
                        Logger.verbose('sending ReindexMessage', loggerCtx);
