...
import { searchGetPricesDocument, searchProductsAdminDocument } from './meilisearch-plugin.e2e-spec';
...
export async function dropMeilisearchIndices(indexPrefix: string) {
    const client = new MeiliSearch({
        host: `${meilisearchHost as string}:${meilisearchPort as string}`,
        apiKey: meilisearchApiKey as string,
    });
    try {
        const indexes = await client.getIndexes();
        for (const index of indexes.results) {
            if (index.uid.startsWith(indexPrefix)) {
                await client.deleteIndex(index.uid);
            }
        }
    } catch (e) {
        // Ignore errors if no indices exist
    }
}
