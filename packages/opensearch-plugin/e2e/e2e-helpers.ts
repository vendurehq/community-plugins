...
import { searchGetPricesDocument, searchProductsAdminDocument } from './opensearch-plugin.e2e-spec';
...
export async function dropOpenSearchIndices(indexPrefix: string) {
    const osClient = new Client({
        node: `${opensearchHost as string}:${opensearchPort as string}`,
    });
    return deleteIndices(osClient, indexPrefix);
}
