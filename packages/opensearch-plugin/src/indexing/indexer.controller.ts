...
@Injectable()
export class OpenSearchIndexerController implements OnModuleInit, OnModuleDestroy {
    private client: Client;
    private asyncQueue = new AsyncQueue('opensearch-indexer', 5);
    ...
    constructor(
        ...
        @Inject(OPENSEARCH_OPTIONS) private options: Required<OpenSearchOptions>,
                        ...
                        operations.push(
                            {
                                ...
                                operation: {
                                    update: {
                                        _id: OpenSearchIndexerController.getId(
                                            variant.id,
                                            ctx.channelId,
                                            languageCode,
                                        ),
    ...
    private async deleteProductOperations(
        ...
        const channels = await this.requestContextCache.get(ctx, 'opensearch-index-all-channels', () =>
        ...
        for (const { id: channelId } of channels) {
            for (const languageCode of uniqueLanguageVariants) {
                operations.push({
                    ...
                    operation: {
                        delete: {
                            _id: OpenSearchIndexerController.getId(-product.id, channelId, languageCode),
                        },
                    },
                });
                if (operations.length >= this.options.reindexBulkOperationSizeLimit) {
    ...
    private async getProductInStockValue(ctx: RequestContext, variants: ProductVariant[]): Promise<boolean> {
        return this.requestContextCache.get(
            ctx,
            `opensearch-index-product-in-stock-${ctx.channelId}-${variants.map(v => v.id).join(',')}`,
            async () => {
